...
static void syndrome(unsigned char *s, const unsigned char *pk, unsigned char *e)
{
	...
	const uint32_t *pk_ptr; 
	const uint32_t *e_ptr = ((uint32_t *) (e_tmp + SYND_BYTES - 1));
	...
	{
		pk_ptr = ((uint32_t *) (pk + PK_ROW_BYTES * i));
	
		b = 0;
		for (j = 0; j < PK_NCOLS/64; j++){
			b ^= vand_u64(vreinterpret_u64_u32(vld1_u32(pk_ptr + 2*j)), vreinterpret_u64_u32(vld1_u32(e_ptr + 2*j)));
		}


		for (k = 0; k < (PK_NCOLS%64 + 7)/8; k++)
			b ^= ((unsigned char *) (pk_ptr + 2*j))[k] & ((unsigned char *) (e_ptr + 2*j))[k];
