...
static inline gf vec_reduce(vec128 *in){
	uint16x4_t tmp1;
	uint32x2_t tmp2;
	uint64x1_t tmp3;
	...
	for(int i = GFBITS-1; i > 0; i--){
		...
		tmp1 = vpaddl_u8(vcnt_u8(vreinterpret_u8_u64(in[i][0]^in[i][1])));
		tmp2 = vpaddl_u16(tmp1);
		tmp3 = vpaddl_u32(tmp2);
		ret ^= (tmp3&1);
		ret <<= 1;
	}
	tmp1 = vpaddl_u8(vcnt_u8(vreinterpret_u8_u64(in[0][0]^in[0][1])));
	tmp2 = vpaddl_u16(tmp1);
	tmp3 = vpaddl_u32(tmp2);
	ret ^= (tmp3&1);
	return ret;
...
static void update(vec128 *in, gf e)
{
	int i;
	...
	for (i = 0; i < GFBITS; i++){
		tmp = (e >> i);
		in[i][0] = (vshr_n_u64(in[i][0], 1)) | vshl_n_u64(in[i][1], 63);	
		in[i][1] = (vshr_n_u64(in[i][1], 1)) | vshl_n_u64(tmp, 63);	
	}
}
